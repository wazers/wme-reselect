...
const ScriptName: string = GM_info.script.name;
...
const options: ScriptOptions = loadOptions();

const selectionHistory: SelectionHistoryItem[] = [];
let rollbackCount = 0;
let rollbackPending: SelectionHistoryItem | SavedSelection | undefined = undefined;
...
function WMEReselect_bootstrap(): void {
...
async function WMEReselect_init(): Promise<void> {
  log("Start");
...
let usableWindow: Window = window;
if (window.unsafeWindow) {
  // Check if unsafeWindow is available, if so use that
  usableWindow = window.unsafeWindow as Window;
}
...
function rollbackToId(selectionId: number): void {
...
function rollbackToSelection(selection: SelectionHistoryItem | SavedSelection): void {
  rollbackCount = 0;
...
function addHotbarIcon() {
  ...
  if (options.showHotbarIcon) {
    ...
    ($(button) as WazeJqueryElement).tooltip({
      trigger: 'hover',
      position: 'down'
    });
...
async function initDatabase(): Promise<void> {
  ...
  request.onupgradeneeded = (event) => {
    ...
    if ((event as IDBVersionChangeEvent).newVersion! >= 1 && (event as IDBVersionChangeEvent).oldVersion < 1) {
  ...
  if ("openDatabase" in window) {
    try {
      ...
      window.openDatabase!(
        "WME-Reselect", // actual database name.  Opens existing or creates.
...
function saveSelection(selection: SelectionHistoryItem): void {
...
function deleteSelection(selection: SavedSelection): void {
  if (!DB) {
    alert("Database not initialized");
    return;
  }
...
function loadSavedSelection(): void {
  ...
  request.onsuccess = (event) => {
    if ((event.target as IDBRequest).result) {
      const results: SavedSelection[] = [...(event.target as IDBRequest).result];
...
////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
function constructSettings(): void {
...
function updateSelectionOptions(): void {
...
function toggleTrackOptions(value: string): void {
...
function getDefaultOptions(): ScriptOptions {
  return {
...
function loadOptions(): ScriptOptions {
...
function saveOptions(options: ScriptOptions): void {
  const optionsJson = JSON.stringify(options);
  localStorage.setItem("Reselect-Options", optionsJson);
}

function toggleBoolean(event: Event): void {
  const target = event.target as HTMLInputElement;
  (options as any)[target.id] = target?.checked;
  saveOptions(options);
}

function changeText(event: Event): void {
  const target = event.target as HTMLInputElement;
  (options as any)[target.id] = target?.value;
  saveOptions(options);
}

function addTextNumberSettings(container: JQuery, title: string, label: string, name: string, step: number = 1): void {
  const currentValue = (options as any)[name];
...
function addBooleanSettings(container: JQuery, title: string, label: string, name: keyof ScriptOptions): void {
  addBooleanSettingsCallback(container, title, label, name, toggleBoolean);
}

function addBooleanSettingsCallback(container: JQuery, title: string, label: string, name: keyof ScriptOptions, clickHandler: (event: Event) => void): void {
...
function addBooleanListSettingsCallback(container: JQuery, title: string, label: string, name: string, value: string, clickHandler?: (event: JQuery.Event, value: string) => void): void {
  const currentlyActive = (options as any)[name].indexOf(value) >= 0;
